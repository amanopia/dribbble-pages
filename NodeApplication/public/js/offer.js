...
const offerUpperContentChildren = [
  ...document.querySelector(".offer-content-upper").children,
];
...
for (let i = 0; i < offerUpperContentChildren.length; i++) {
  ...
  offerUpperContentChildren[i].style.transitionDelay =
    serviceAnimationDelay + "s";
  serviceAnimationDelay += 0.1;
}
// IMAGE -- OBSERVER FUNCTION
const offerUpperContentObserver = new IntersectionObserver(
  (entries) => {
...
//! ------------------ BOTTOM SECTION ------------------
const offerBottomSectionChildren = [
  ...document.querySelector(".offers").children,
];
...
for (const element of offerBottomSectionChildren) {
  ...
  for (let j = 0; j < childArr.length; j++) {
    ...
    childArr[j].style.transitionDelay =
      serviceAnimationDelay + delayDuration + "s";
...
const bottomOfferContainerObserver = new IntersectionObserver(
  (entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
